    ...
    <nav class="nav-menu-wrapper">
      <div class="nav-menu-content">
        ...
        <div class="buttons-wrapper">
          <div class="button">
            <div class="active" @click="goToAppointment()">
              <img
                src="~/assets/images/calendar-regular.svg"> Create Appointment
            </div>
          </div>
          <div class="button">
            <div class="active" @click="$emit('openAppointmentsModal')">
              <img
                src="~/assets/images/calendar-regular.svg"> Edit Appointments
            </div>
          </div>
          <div v-if="userInfo.role === 'manager'" class="button">
            <div class="active" @click="$emit('openEmployeeModal')">
              <img
                src="~/assets/images/users-solid.svg"> Employees
            </div>
          </div>
        </div>
        <div class="extras">
          <p>{{ userInfo.name }}</p>
          <div v-if="userInfo.name" class="cursor-pointer" @click="logout()">
            <img
              src="~/assets/images/door-open-solid.svg">
          </div>
          <a v-if="!userInfo.name" href="/login"><img
              src="~/assets/images/door-open-solid.svg"></a>
        </div>
      </div>
    </nav>
  </div>
  ...
  methods: {
    async logout () {
      try {
        ...
        await this.$axios.delete(`http:https://easybeauty.somee.com/v1/api/Login/logout?cookie=${cookie}`);
