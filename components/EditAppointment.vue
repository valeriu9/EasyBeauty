  ...
  methods: {
    ...
    async loadAppointmentsById (userId) {
      try {
        ...
        const appointments = await this.$axios.get(`https://easybeauty.somee.com/v1/api/Appointment?employeeId=${userId}`);
    ...
    async onSubmit () {
      try {
        ...
        const res = await this.$axios.put(`https://easybeauty.somee.com/v1/api/Appointment?id=${this.selectedEvent.id}&cookie=${this.cookie}`, objToSend);
        if (res.data.error) {
    ...
    async onDecline () {
      try {
        const res = await this.$axios.delete(`https://easybeauty.somee.com/v1/api/Appointment?id=${this.selectedEvent.id}&cookie=${this.cookie}`);
        if (res.data.error) {
          window.alert(res.data.error);
        } else {
          this.eventHasChanges = false;
