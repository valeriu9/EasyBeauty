  ...
  methods: {
    ...
    async loadAppointmentsById (userId) {
      try {
        ...
        const appointments = await this.$axios.get(`${process.env.BASE_URL}/Appointment?employeeId=${userId}`);
    ...
    async onSubmit () {
      try {
        ...
        const res = await this.$axios.put(`${process.env.BASE_URL}/Appointment?id=${this.selectedEvent.id}&cookie=${this.cookie}`, objToSend);
        if (res.data.error) {
    ...
    async onDecline () {
      try {
        const res = await this.$axios.delete(`${process.env.BASE_URL}/Appointment?id=${this.selectedEvent.id}&cookie=${this.cookie}`);
        if (res.data.error) {
          window.alert(res.data.error);
        } else {
          this.eventHasChanges = false;
